---
id: yd/readme/outbound_notes
title: Outbound notes lane
domain: artifacts
lane: Outbound_Notes
status: active
version: 1.0
owner: dale
created: 2025-12-08
updated: 2025-12-08
provenance: relationship-first initializer
tags: [outbound, email, sms, cards, artifacts]
constraints: {"mece_required": true, "provenance_required": true}
---
# Outbound notes lane

## Note schema (YAML fields)
- id, recipients, channel (email/sms/card), date_target/sent
- tone, purpose, draft_text, final_text
- attachments_refs, follow_up_date, status
- privacy_level (public/private/confidential)

## Notes

## Next Steps

## Related
- [[00_Index/People/index.md|People]]
- [[12_Events/Occasions/2025/index.md|Occasions 2025]]\n- [[12_Events/Gifts/2025/index.md|Gifts 2025]]\n- [[11_Artifacts/Outbound_Notes/2025/index.md|Notes 2025]]


# Summary


## Rationale


## Links


## Next steps